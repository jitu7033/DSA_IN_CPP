...
struct LazySGT {
    ...
    void pushdown(int index, int start, int end){
        if(lazy[index]){
            ...
            if((mid-start+1)%2==0){
            	...
            	Update new_update = Update(-updates[index].val);
            	apply(2 * index + 1, mid + 1, end, new_update);
            }
    ...
    int intersection(int l1, int r1, int l2, int r2){
    	...
    	int intersectionL = min(l1,r1);
    	int intersectionR = max(r1,r2);
    	if(intersectionR>=intersectionL){
    		return intersectionR-intersectionL+1;
    	}
    	else{
    		return 0;
    	}
    ...
    void update(int start, int end, int index, int left, int right, Update& u) {  // Never Change this
        ...
        int intersect = intersection(left,right,start,end);
        if(intersect%2==0){
        	 update(mid+1,end, 2 * index+1, left, right, u);
        }
        else{

        	Update new_update = Update(-u.val);
        	 update(mid+1,end, 2 * index+1, left, right, new_update);
        }
        tree[index].merge(tree[2 * index], tree[2 * index + 1]);
    }
