...
struct Node{
    ll segSum ,prefSum, suffSum, maxSum;
    Node(){
        segSum = prefSum=suffSum=maxSum=0;
    }
    Node(ll p1){
        segSum = p1;
    }
    void merge(Node &l, Node &r){
        ...
        prefSum = max(l.prefSum,l.segSum+r.segSum);
        suffSum = max(r.suffSum,l.segSum+r.segSum);
        maxSum = max({l.maxSum,r.maxSum,l.suffSum+r.prefSum});
...
struct update2 {
    ...
    void apply(Node &a, int start, int end){
        ll ss = ((end-start+1)*val);

        if(val>0){
            a.segSum = a.prefSum = a.suffSum= a.maxSum = ss;
        }
        else{
            a.segSum = ss;
            a.prefSum = a.suffSum = a.maxSum = 0;
        }
...
void solve(){
    ...
    cin>>n>>m;
    vector<ll>arr(n,0);
    LazySGT<Node,update2>sgt = LazySGT<Node,update2>(n,arr);
    while(m--){
        int l,r,v;
        cin>>l>>r>>v;
        r--;
        sgt.make_update(l,r,v);
        cout<<sgt.make_query(0,n-1).maxSum<<endl;
    }
}
int main(){
    solve();
